...
- (id) transformedValue:(NSString *)value
{
    ...
    // Can be -1 when empty
    signed short keyCode = [value intValue];
	...
	if ( unmappedString != nil ) return [modsString stringByAppendingString:unmappedString];
	...
	if ( currentLayoutKind == kKLKCHRKind )
	{
        ...
        return [modsString stringByAppendingString:keyString];
	}
...
- (id) reverseTransformedValue:(id)value
{
    ...
    return value; //we're setting the value directly - no need to transform the value
}
